...
async GetFilterCategory(value) {
  ...
  if(value.status != undefined && value.categoryId === undefined && value.type === undefined){
    ...
    var endPoint = new Request(`${SharedData.BASE_URL}components/products?parentId=`+ value.categoryId,
    { method: 'GET',}
    )
  }else if(value.type != undefined && value.status === undefined && value.categoryId === undefined){
    var endPoint = new Request(`${SharedData.BASE_URL}components/products?type=`+ value.type,
    { method: 'GET',}
    )
  }else if(value.status != undefined && value.type != undefined && value.categoryId === undefined){
    var endPoint = new Request(`${SharedData.BASE_URL}components/products?state=`+ value.status + '&type=' + value.type,
    { method: 'GET',}
    )
  }else if(value.status != undefined && value.categoryId != undefined && value.type === undefined){
    var endPoint = new Request(`${SharedData.BASE_URL}components/products?state=`+ value.status + '&parentId=' + value.categoryId,
    { method: 'GET',}
    )
  }else if(value.type != undefined && value.categoryId != undefined && value.status === undefined){
    var endPoint = new Request(`${SharedData.BASE_URL}components/products?type=`+ value.type + '&parentId=' + value.categoryId,
    { method: 'GET',}
    )
  }
  else{
